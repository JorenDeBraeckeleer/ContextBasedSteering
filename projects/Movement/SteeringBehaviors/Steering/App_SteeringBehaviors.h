...
private:
	...
	struct ImGui_Agent
	{
		...
		int SelectedBehavior = int(BehaviorTypes::ContextTutorial);
	...
	std::vector<std::pair<Elite::Vector2, float>> m_AllObstacles;
	...
	ImGui_Agent App_SteeringBehaviors::AddAgent(BehaviorTypes behaviorType = BehaviorTypes::ContextTutorial, int targetId = -1, bool autoOrient = true, float mass = 1.f, float maxSpd = 7.f);
