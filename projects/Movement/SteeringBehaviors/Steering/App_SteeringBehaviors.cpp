...
{
	AddAgent(BehaviorTypes::ContextTutorial, -1);
...
#pragma region UI
	//UI
	{
		...
		if (ImGui::Button("Add Agent"))
			AddAgent(BehaviorTypes::ContextWander);
		...
		for (UINT i = 0; i < m_AgentVec.size(); ++i)
		{
			...
			if (ImGui::CollapsingHeader(headerName))
			{
				...
				if (ImGui::Combo("", &a.SelectedBehavior, "ContextTutorial\0ContextImproved\0ContextSeek\0ContextFlee\0ContextArrive\0ContextWander", 6))
				{
...
void App_SteeringBehaviors::SetAgentBehavior(ImGui_Agent& a)
{
	...
	{
	case BehaviorTypes::ContextTutorial:
		a.pBehavior = new Context(m_Obstacles);
		break;
	case BehaviorTypes::ContextImproved:
		a.pBehavior = new ContextImproved(m_Obstacles);
		break;
	case BehaviorTypes::ContextSeek:
		a.pBehavior = new ContextSeek(m_AllObstacles);
		break;
	case BehaviorTypes::ContextFlee:
		a.pBehavior = new ContextFlee(m_AllObstacles);
		break;
	case BehaviorTypes::ContextArrive:
		a.pBehavior = new ContextArrive(m_AllObstacles);
		break;
	case BehaviorTypes::ContextWander:
		a.pBehavior = new ContextWander(m_AllObstacles);
		break;
	default:
			break;
	}
...
void App_SteeringBehaviors::AddObstacle()
{
	...
	if (positionFound)
	{
		...
		m_AllObstacles.push_back(std::pair<Elite::Vector2, float>(pos, radius));
	}
}

//void App_SteeringBehaviors::AddRaceCourse()
//{
//	float radius{ 2.f };
//	Elite::Vector2 pos{ -m_TrimWorldSize, -m_TrimWorldSize };
//
//	size_t amountOfBarriers{ 132 };
//	for (size_t idx{}; idx < amountOfBarriers; ++idx)
//	{
//		if (idx < 33) pos.x += radius * 1.5f;
//		else if (idx < 66) pos.y += radius * 1.5f;
//		else if (idx < 99) pos.x -= radius * 1.5f;
//		else pos.y -= radius * 1.5f;
//
//		m_Obstacles.push_back(new Obstacle(pos, radius));
//		m_AllObstacles.push_back(std::pair<Elite::Vector2, float>(pos, radius));
//	}
...
//	pos = { -10.f, -10.f };
//
//	size_t amountOfWalls{ 58 };
//	for (size_t idx{}; idx < amountOfWalls; ++idx)
//	{
//		if (idx < 20) pos.x += radius * 1.5f;
//		else pos.y += radius * 1.5f;
//
//		m_Obstacles.push_back(new Obstacle(pos, radius));
//		m_AllObstacles.push_back(std::pair<Elite::Vector2, float>(pos, radius));
...
//	for (size_t idx{}; idx < amountOfWalls; ++idx)
//	{
//		if (idx < 44) pos.y -= radius * 1.5f;
//		else if (idx < 85) pos.x -= radius * 1.5f;
//		else if (idx < 110) pos.y += radius * 1.5f;
//		else if (idx < 128) pos.x += radius * 1.5f;
//		else pos.y += radius * 1.5f;
//
//		m_Obstacles.push_back(new Obstacle(pos, radius));
//		m_AllObstacles.push_back(std::pair<Elite::Vector2, float>(pos, radius));
